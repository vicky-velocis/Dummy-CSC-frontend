...
export const createUpdatePriceList = async (state, dispatch, action) => {
  ...
  if (action === "CREATE") {
    try {
      ...
      if(response){
        dispatch(setRoute(`/egov-store-asset/acknowledgement?screen=priceList&mode=create&code=123456`));
       }
    } catch (error) {
    ...
    try {
      ...
      // dispatch(setRoute(acknowledgementUrl));
      if(response){
        dispatch(setRoute(`/egov-store-asset/acknowledgement?screen=priceList&mode=update&code=123456`));
       }
    } catch (error) {
      furnishPriceListData(state, dispatch);
    }
