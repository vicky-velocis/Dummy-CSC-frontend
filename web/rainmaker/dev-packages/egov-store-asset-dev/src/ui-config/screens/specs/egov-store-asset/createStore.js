import {
  ...
  const  textFields = ["storeCode","storeName","departmentName","storeDescription","billingAddress","deliveryAddress","inChargeEmployee","contactNumber1","contactNumber2","email","officeLocation"];
   ...
   // document.getElementById('central-store').removeAttribute('checked');
    ...
    for(let i= 0 ; i < textFields.length ; i++ ){
      if(state.screenConfiguration.screenConfig.createStore.components.div.children.formwizardFirstStep.children.formDetail.children.cardContent.children.addStoreDetails.children[textFields[i]].props.value){
      dispatch(handleField(
        "createStore",
