...
export const getTextToLocalMapping = (label) => {
  ...
  switch (label) {
    ...
    case "Search Results for SUSV":
      return getLocaleLabels(
        "Search Results for SUSV",
        "NULM_SUSV_SEARCH_RESULTS_TABLE_HEADING",
        localisationLabels
      );
  }
};
...
export const searchResults = {
  ...
  props: {
    ...
    title: getTextToLocalMapping("Search Results for SUSV"),
    options: {
      filter: false,
      download: false,
      responsive: "stacked",
...
const onRowClick = (rowData) => {
  ...
  window.location.href = `view-susv?tenantId=${tenantId}&applicationNumber=${rowData[0]}&status=${rowData[2]}`;
};
