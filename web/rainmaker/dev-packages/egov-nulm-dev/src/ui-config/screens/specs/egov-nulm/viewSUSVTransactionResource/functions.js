...
export const createUpdatePO = async (state, dispatch, action ,status) => {
  ...
  let NulmTransactionRequest = get(
    state.screenConfiguration.preparedFinalObject,
    "NulmTransactionRequest",
    []
  );
  ...
  let queryObject = [{ key: "tenantId", value: tenantId }];
   ...
   NulmTransactionRequest.applicationStatus = status;
  ...
  if (action === "CREATE") {
    try {
      const response = await httpRequest(
        "post",
        "/nulm-services/v1/susv/transaction/_create",
        "",
        queryObject,
        requestBody
      );
       if(response){
        dispatch(setRoute(`/egov-nulm/acknowledgement?screen=susvtransaction&mode=create&code=${response.ResponseBody.uuid}`));
      }
    ...
    try {
      const response = await httpRequest(
        "post",
        "/nulm-services/v1/susv/transaction/_update",
        "",
        queryObject,
        requestBody
      );
       if(response){
        dispatch(setRoute(`/egov-nulm/acknowledgement?screen=susvtransaction&mode=update&code=${response.ResponseBody.uuid}`));
       }
