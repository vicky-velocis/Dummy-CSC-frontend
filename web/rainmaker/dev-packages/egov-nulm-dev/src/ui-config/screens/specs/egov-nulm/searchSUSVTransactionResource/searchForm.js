...
const resetFields = (state, dispatch) => {
  const textFields = ["transactionType","fromDate","toDate"];
  for (let i = 0; i < textFields.length; i++) {
    if (
      `state.screenConfiguration.screenConfig.search-susv-transaction.searchForm.children.cardContent.children.searchFormContainer.children.${textFields[i]}.props.value`
    ) {
      dispatch(
        handleField(
  ...
  searchFormContainer: getCommonContainer({ 
    transactionType: {
      ...getTextField({
        ...
        placeholder: {
          labelName: "Enter Transaction Type",
