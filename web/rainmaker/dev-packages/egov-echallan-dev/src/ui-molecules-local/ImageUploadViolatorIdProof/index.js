  ...
  return (
    <div className={`${className} upload-placeholder`} style={hide ? { visibility: "hidden" } : {}}>
      <FilePicker inputProps={{ ...inputProps, multiple: false }} handleimage={onFilePicked}>
        ...
        <Label label="EC_CS_COMMON_VIOLATOR_ID_PROOF_UPLOAD_PHOTOS" labelStyle={labelStyle} fontSize="12px" />
      </FilePicker>
    </div>
  );
};

class ImageUploadViolatorIdProof extends Component {
