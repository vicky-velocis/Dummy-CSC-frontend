...
function FeesEstimateCard(props) {
  ...
  const amount = totalAmount(estimate.fees);

  const total=(amount).toFixed(2).replace(/(\d)(?=(\d{2})+\d\.)/g, '$1,')
            ...
            {estimate.fees.map((fee, key) => {
              ...
              let textRight = fee.value ? (
                <Grid xs={4} align="right">
                  <LabelContainer
                    labelName={fee.value}
                    labelKey={(fee.value).toFixed(2).replace(/(\d)(?=(\d{2})+\d\.)/g, '$1,')}
            ...
            <Grid item xs={6}>
              <Typography variant="body2">
                <LabelContainer
                  ...
                  labelKey="EC_TL_COMMON_TOTAL_AMT"
                />
              </Typography>
            </Grid>
            <Grid
