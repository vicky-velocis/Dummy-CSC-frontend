...
export const callPGService = async (state, dispatch) => {
  ...
  const gateway = get(state, "screenConfiguration.preparedFinalObject.eChallanDetail.paymentGateway",'');
...
if(gateway != ''){
  let callbackUrl = `${
    process.env.NODE_ENV === "production"
      ...
      : window.origin
    }/egov-echallan/paymentRedirectPage`;
  try {
    const queryObj = [
  ...
  dispatch(toggleSnackbar(true,
    { labelName: "Select Gateway to Proceed", labelKey: "EC_SELECT_GATEWAY_LABEL" },
    "warning"));
}
...
const moveToSuccess = (dispatch, receiptNumber) => {
  ...
  dispatch(
    setRoute(
      `${appendUrl}/egov-echallan/acknowledgement?purpose=${purpose}&status=${status}&applicationNumber=${applicationNo}&tenantId=${tenantId}&secondNumber=${receiptNumber}`
    )
  );
};
...
const updatePayAction = async (
  ...
  try {
    ...
    set(response, "echallanDetails[0].action", "PAY");
...
export const footer = getCommonApplyFooter({
  submitButton: {
    ...
    roleDefination: {
      ...
      roles: ["NOC_CEMP","CITIZEN", "challanSI"],
      action: "PAY"
    },
  ...
  makePayment: {
    ...
    visible: true //process.env.REACT_APP_NAME === "Citizen" ? true : false
  }
});
