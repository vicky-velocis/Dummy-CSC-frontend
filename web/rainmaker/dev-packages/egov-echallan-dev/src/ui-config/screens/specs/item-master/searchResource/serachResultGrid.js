...
import "./index.css"
import {
  sortByEpoch, getEpochForDate, getTextToLocalMappingItemMaster, checkForRole, getTextToLocalSeizedItemDetailHeader
} from "../../utils";
import {
...
import {
  getLocaleLabels, getTransformedLocalStorgaeLabels
          ...
          customBodyRender: (value, tableMeta, updateValue) => {
            return (
              <React.Fragment>
                ...
                } style={{backgroundColor:"unset", boxShadow: "unset", border: "unset"}}>
                  <span>
...
const onRowClick = (rowData, isDeleteCalled) => {
  ...
  if (isEditAllowed) {
    ...
    store.dispatch(prepareFinalObject('ItemMaster.itemUuid', rowData[0]));
    store.dispatch(prepareFinalObject('ItemMaster.itemName', rowData[1]));
    store.dispatch(prepareFinalObject('ItemMaster.description', rowData[2]));
    ...
    store.dispatch(
      toggleSnackbar(
        true,
        {
          ...
          labelKey: "EC_ITEM_MASTER_SUBMISSION_FAILD"
        },
        "warning"
     ));
  }
