...
import { previousButton, submitButton, nextButton, changeStep, moveToSuccess,dulicateCopyMoveToSuccess, DETAILS_STEP, DOCUMENT_UPLOAD_STEP, SUMMARY_STEP } from "../rented-properties/applyResource/footer";
...
const callBackForNextDuplicate = async(state, dispatch) => {
    ...
    isFormValid = await applyDuplicateOwnershipTransfer(state, dispatch);
      if (isFormValid) {
          dulicateCopyMoveToSuccess(rentedData, dispatch, "DUPLICATECOPYRP");
      }
    }
    if(activeStep !== SUMMARY_STEP) {
        if (isFormValid) {
