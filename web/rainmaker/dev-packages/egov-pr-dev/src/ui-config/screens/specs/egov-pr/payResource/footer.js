...
const moveToSuccess = (dispatch, receiptNumber) => {
  ...
  dispatch(
    setRoute(
      `${appendUrl}/egov-pr/acknowledgement?purpose=${purpose}&status=${status}&applicationNumber=${applicationNo}&tenantId=${tenantId}&secondNumber=${receiptNumber}`
    )
  );
};
...
const updatePayAction = async (
  ...
  try {
    ...
    set(response, "PublicRelations[0].PublicRelationDetails.action", "PAY");
    response = await httpRequest(
      "post",
      "/PublicRelation-services/v1/_update",
      "",
      [],
      {
        PublicRelations: get(response, "PublicRelations", [])
      }
    );
    if (get(response, "PublicRelations", []).length > 0) {
