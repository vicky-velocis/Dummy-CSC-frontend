...
const getMdmsData = async (state, dispatch) => {
  let tenantId = get(
    ...
    "PETNOC[0].PETNOCDetails.propertyDetails.address.city"
  );
  let mdmsBody = {
    MdmsCriteria: {
      tenantId: tenantId,
...
const callBackForNext = async (state, dispatch) => {
  let activeStep = get(
    state.screenConfiguration.screenConfig["petapply"],
  ...
  //alert("Entered in active step != 3 " + activeStep + " : "+ isFormValid)
    if (isFormValid) {
     //alert("Entered in form valid yes " )
      let responseStatus = "success";
      if (activeStep === 1) {
        //prepareDocumentsUploadData(state, dispatch);
		//alert("Prepare Documents call")
      }
      if (activeStep === 2) {
	  ...
	  //alert("active step zero" + activeStep + " state "  + state + " dispatch " + dispatch)
